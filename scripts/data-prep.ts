...
( async () => {
    ...
    const csvFilePath = path.resolve(__dirname, '../data/metros.csv')

    const csvContent = fs.readFileSync(csvFilePath, { encoding: 'utf-8' })
    const toNumber = (s: string) => Number(s.replace(/[$,]/g, ''))

    const metroGeojson = await new Promise((resolve, reject) => {
       ...
       parse(csvContent, {
            ...
            Promise.all(result.map(async (c) =>  {
    ...
    fs.writeFile('./data/metros.json', JSON.stringify(metroGeojson, null, 2), (err) => {
        if(err) console.error(err);
        else console.log("Wrote Cities geojson")
    })
})()
